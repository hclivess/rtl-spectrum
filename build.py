#!/usr/bin/env python3
"""
Build a self-contained rtl-spectrum distribution (no Python required on the target) using PyInstaller,
then package it as
    dist/rtl-spectrum-<version>-<os>-<arch>.zip      (Windows)
    dist/rtl-spectrum-<version>-<os>-<arch>.tar.gz   (Linux / macOS, keeps exec bits)
...
APP_DESCRIPTION = "Wideband receiver for RTL-SDR: listen, sweep, scan-and-record, decode aircraft"   # goes into the Windows version resource

BUILD_DIR = os.path.join(ROOT, "build")
DIST_DIR = os.path.join(ROOT, "dist")
EXTRA_FILES = ("README.md", "LICENSE", "icon.ico", "icon.png", "thumb.png")
...
def pyinstaller_command() -> list:
    cmd = [
        ...
        # --- app-specific: collect the native / data-heavy packages you actually use, e.g.
